...
contract LPNFTStakeFarm is Context, ReentrancyGuard, ILPNFTStakeFarm {
  ...
  /**
   * @dev The BORROW token used to return tokens
   */
   ...
   * @param borrowToken_ The address of the BORROW token, used to return tokens
  ...
  function _returnTokens(address recipient) private {
    ...
    if (borrowBalance > 0) {
      borrowToken.safeTransfer(recipient, borrowBalance);
    }
  }
