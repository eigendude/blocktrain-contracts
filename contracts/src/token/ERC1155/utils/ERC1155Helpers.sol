...
import {IERC1155Batched} from "../../../interfaces/token/ERC1155/extensions/IERC1155Batched.sol";
...
library ERC1155Helpers {
  ...
  /**
   * @dev Hard cap on batch size to bound gas and calldata/L1 data fees
   */
  ...
  /**
   * @dev Check that a batch size does not exceed {MAX_BATCH}
   *
   * This caps user-controlled batch sizes to bound gas and calldata/L1 fees.
   */
  function checkBatchSize(uint256[] memory tokenIds) internal pure {
    if (tokenIds.length > MAX_BATCH) {
      revert IERC1155Batched.BatchTooLarge(tokenIds.length, MAX_BATCH);
    }
  }
}
