...
import {
  TEST_BORROW_STABLE_STAKER_CONTRACT,
...
async function getAddressBook(networkName: string): Promise<AddressBook> {
  return {
    ...
    borrowInterestFarm: await getContractAddress(
      "borrowInterestFarm",
      BORROW_INTEREST_FARM_CONTRACT,
      networkName,
    ),
    borrowLpNftStakeFarm: await getContractAddress(
      "borrowLpNftStakeFarm",
      BORROW_LPNFT_STAKE_FARM_CONTRACT,
      networkName,
    ),
    borrowLpSftLendFarm: await getContractAddress(
      "borrowLpSftLendFarm",
      BORROW_LPSFT_LEND_FARM_CONTRACT,
      networkName,
    ),
    borrowStablePool: await getContractAddress(
      "borrowStablePool",
      BORROW_STABLE_POOL_CONTRACT,
      networkName,
    ),
    borrowStablePooler: await getContractAddress(
      "borrowStablePooler",
      BORROW_STABLE_POOLER_CONTRACT,
      networkName,
    ),
    borrowStablePoolFactory: await getContractAddress(
      "borrowStablePoolFactory",
      BORROW_STABLE_POOL_FACTORY_CONTRACT,
      networkName,
    ),
    borrowStableSwapper: await getContractAddress(
      "borrowStableSwapper",
      BORROW_STABLE_SWAPPER_CONTRACT,
      networkName,
    ),
    borrowToken: await getContractAddress(
      "borrowToken",
      BORROW_TOKEN_CONTRACT,
      networkName,
    ),
    ...
    testBorrowStableStakerContract: await getContractAddress(
      "testBorrowStableStakerContract",
      TEST_BORROW_STABLE_STAKER_CONTRACT,
      networkName,
    ),
