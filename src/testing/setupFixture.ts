...
async function setupFixture(
  ...
  const borrowInterestFarmContract = new ethers.Contract(
    addressBook.borrowInterestFarm!,
    erc20InterestFarmAbi,
    beneficiary,
  );
  const borrowLpNftStakeFarmContract = new ethers.Contract(
    addressBook.borrowLpNftStakeFarm!,
    uniV3StakeFarmAbi,
    beneficiary,
  );
  const borrowLpSftLendFarmContract = new ethers.Contract(
    addressBook.borrowLpSftLendFarm!,
    lpSftLendFarmAbi,
    beneficiary,
  );
  const borrowStablePoolContract = new ethers.Contract(
    addressBook.borrowStablePool!,
    uniswapV3PoolAbi,
    beneficiary,
  );
  const borrowStablePoolerContract = new ethers.Contract(
    addressBook.borrowStablePooler!,
    gameTokenPoolerAbi,
    beneficiary,
  );
  const borrowStablePoolFactoryContract = new ethers.Contract(
    addressBook.borrowStablePoolFactory!,
    uniV3PoolFactoryAbi,
    beneficiary,
  );
  const borrowStableSwapperContract = new ethers.Contract(
    addressBook.borrowStableSwapper!,
    gameTokenSwapperAbi,
    beneficiary,
  );
  const borrowTokenContract = await hardhat_re.ethers.getContractAt(
    borrowTokenAbi,
