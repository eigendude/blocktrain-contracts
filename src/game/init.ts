...
async function initializeGame(
  ...
  if (networkName == "localhost") {
    ...
    addressBook = {
      ...
      borrowInterestFarm: contracts[BORROW_INTEREST_FARM_CONTRACT]["address"],
      borrowLpNftStakeFarm:
        contracts[BORROW_LPNFT_STAKE_FARM_CONTRACT]["address"],
      borrowLpSftLendFarm:
        contracts[BORROW_LPSFT_LEND_FARM_CONTRACT]["address"],
      borrowStablePooler: contracts[BORROW_STABLE_POOLER_CONTRACT]["address"],
      borrowStablePoolFactory:
        contracts[BORROW_STABLE_POOL_FACTORY_CONTRACT]["address"],
      borrowStableSwapper: contracts[BORROW_STABLE_SWAPPER_CONTRACT]["address"],
      borrowToken: contracts[BORROW_TOKEN_CONTRACT]["address"],
    ...
    addressBook.borrowStablePool = await uniswapV3FactoryContract.getPool(
      addressBook.borrowToken!,
  ...
  // Borrow more BORROW, if needed
  if (debtBalance < INITIAL_BORROW_AMOUNT) {
    ...
    await liquidityForgeContract.borrowBorrow(
      ...
      INITIAL_BORROW_AMOUNT - debtBalance, // amount
