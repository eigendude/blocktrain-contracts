...
describe("Bureau 3: Liquidity Forge", () => {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Spec: Borrow BORROW from LiquidityForge
  //////////////////////////////////////////////////////////////////////////////
  ...
  it("should check YIELD LP-SFT LPYIELD balance", async function (): Promise<void> {
    this.timeout(60 * 1000);

    // Calculate DeFi properties
    const lpYieldValue: string = ethers.formatUnits(
      INITIAL_LPYIELD_AMOUNT / BigInt(1 / INITIAL_BORROW_PRICE),
      LPYIELD_DECIMALS,
    );
  ...
  it("should borrow BORROW from LiquidityForge", async function (): Promise<void> {
    this.timeout(60 * 1000);
    ...
    const borrowValue: string = ethers.formatUnits(
      INITIAL_BORROW_AMOUNT / BigInt(1 / INITIAL_BORROW_PRICE),
      BORROW_DECIMALS,
    );

    console.log(
      `    Borrowing BORROW: ${ethers.formatUnits(
        INITIAL_BORROW_AMOUNT,
        BORROW_DECIMALS,
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Spec: Check LP-SFT balances after borrowing BORROW
  //////////////////////////////////////////////////////////////////////////////

  it("should check LP-SFT balances after borrowing BORROW", async function (): Promise<void> {
    ...
    chai.expect(borrowAmount).to.equal(INITIAL_BORROW_AMOUNT);

    const debtAmount: bigint = await defiManagerContract.debtBalance(
      LPYIELD_LPNFT_TOKEN_ID,
    );
    chai.expect(debtAmount).to.equal(INITIAL_BORROW_AMOUNT);
  });

  //////////////////////////////////////////////////////////////////////////////
  // Spec: Test withdrawing LP-SFT before BORROW loan is repaid
  //////////////////////////////////////////////////////////////////////////////

  it("should fail to withdraw LP-SFT before BORROW loan is repaid", async function (): Promise<void> {
    this.timeout(60 * 1000);

    const { noLpSftContract } = beneficiaryContracts;

    // Attempt to withdraw LP-SFT before BORROW loan is repaid
    try {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Spec: Approve LiquidityForge to spend BORROW
  //////////////////////////////////////////////////////////////////////////////
  ...
  it("should repay BORROW loan", async function (): Promise<void> {
    ...
    const { liquidityForgeContract } = beneficiaryContracts;
    // Repay BORROW loan
    await liquidityForgeContract.repayBorrow(
      LPYIELD_LPNFT_TOKEN_ID, // tokenId
      INITIAL_BORROW_AMOUNT, // amount
    );
  });
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Spec: Check LP-SFT balances after repaying BORROW
  //////////////////////////////////////////////////////////////////////////////
  ...
  it("should check LP-SFT balances after repaying BORROW", async function (): Promise<void> {
    ...
    chai.expect(borrowAmount).to.equal(0n);
