...
const BORROW_AMOUNT: bigint = ethers.parseUnits("1000", BORROW_DECIMALS); // 1,000 BORROW
...
describe("ERC20Nontransferable", () => {
  ...
  it("should grant ERC20_ISSUER_ROLE for BORROW", async function () {
    this.timeout(60 * 1000);

    const { borrowTokenContract } = contracts;

    // Grant issuer role to deployer
    const tx: ethers.ContractTransactionResponse = await (
      borrowTokenContract.connect(deployer) as ethers.Contract
    ).grantRole(ERC20_ISSUER_ROLE, await deployer.getAddress());
    await tx.wait();
  });
  ...
  it("should mint BORROW", async function (): Promise<void> {
    this.timeout(60 * 1000);
    ...
    const tx: ethers.ContractTransactionResponse = await (
      borrowTokenContract.connect(deployer) as ethers.Contract
    ).mint(await deployer.getAddress(), BORROW_AMOUNT);
    await tx.wait();
  });
  ...
  it("should succeed to transfer BORROW", async function () {
    this.timeout(60 * 1000);
    ...
    const tx: ethers.ContractTransactionResponse = await (
      borrowTokenContract.connect(deployer) as ethers.Contract
    ).transfer(beneficiaryAddress, BORROW_AMOUNT);
    await tx.wait();
  });
