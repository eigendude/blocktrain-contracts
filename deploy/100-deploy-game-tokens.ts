...
const func: DeployFunction = async (hardhat_re: HardhatRuntimeEnvironment) => {
  ...
  if (addressBook.borrowToken && networkName !== "localhost") {
    console.log(`Using ${BORROW_TOKEN_CONTRACT} at ${addressBook.borrowToken}`);
  } else {
    console.log(`Deploying ${BORROW_TOKEN_CONTRACT}`);
    const tx = await deployments.deploy(BORROW_TOKEN_CONTRACT, {
      ...opts,
      args: [
        deployer, // owner
      ],
    });
    addressBook.borrowToken = tx.address as `0x${string}`;
  }
